...
public class BankCardUtil {
     ...
     * @return
     */
    public static Map<String, Object> bankCardVerification(String accountNo) {
        String res = null;
        String url = "https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?cardNo=" + accountNo + "&cardBinCheck=true";
...
//                	System.out.println(((Map.Entry)map).getKey()+"     " + ((Map.Entry)map).getValue());
                if (((Map.Entry) map).getKey().equals("validated")) {
                    maper.put("validated", ((Map.Entry) map).getValue().toString());
                }
                if (((Map.Entry) map).getKey().equals("bank")) {
                    maper.put("bank", bankBearch(((Map.Entry) map).getValue().toString()));
                }
                if (((Map.Entry) map).getKey().equals("cardType")) {
                    String cardType = ((Map.Entry) map).getValue().toString();
                    if (cardType.equals("DC")) maper.put("cardType", "储蓄卡");
                    if (cardType.equals("CC")) maper.put("cardType", "信用卡");
                    if (cardType.equals("SCC")) maper.put("cardType", "准贷记卡");
                    if (cardType.equals("PC")) maper.put("cardType", "预付费卡");
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private static String bankBearch(String string) {
        Map maps = (Map) JSON.parse(TypeBank);
        for (Object map : maps.entrySet()) {
            if (((Map.Entry) map).getKey().equals(string)) {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Object> str = bankCardVerification("6225680421001165184");
