...
*/
@RestController
@RequestMapping("/api/v1/bookPreorder")
@Slf4j
public class RestBookPreorderController {
   @Autowired
   ...
   @ApiOperation(value = "添加预购书籍", tags = "书店接口", notes = "添加预购书籍")
   public RestResponseBean add(@RequestBody BookPreorder bookPreorder) {
	   Result<BookPreorder> result = new Result<BookPreorder>();
	   User sysUser = (User) SecurityUtils.getSubject().getPrincipal();
	   bookPreorder.setUid(sysUser.getId());
