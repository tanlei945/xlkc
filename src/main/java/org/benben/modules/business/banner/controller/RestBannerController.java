...
import org.benben.modules.business.banner.vo.BannersVo;
import org.benben.modules.business.banner.vo.ImgUrl;
...
public class RestBannerController {
	 ...
	 * @description 展示轮播图接口
	 * @method Get
	 * @url https://192.168.1.125:8081/xlkc-boot/api/v1/banner/getBanner
	 * @return {"code": 1,"msg": "操作成功","time": "1561168799591","data": {"id": "1","type": 1,"imgUrl": "{\"1\":\"https://xinlikecheng1.oss-cn-beijing.aliyuncs.com/1560417582773.png\",\"2\":\"https://xinlikecheng1.oss-cn-beijing.aliyuncs.com/1560417586340.png\",\"297e017e6ac9c038016ac9c038ef0000\":\"https://xinlikecheng1.oss-cn-beijing.aliyuncs.com/1560417589174.png\"}","createTime": "2019-05-31 14:39:44","createBy": null,"updateTime": "2019-06-13 16:11:34","updateBy": "superadmin"}}
	 * @param type int 1/首页轮播图，2/论坛轮播图
	 * @remark 轮播图接口
	 * @number 99
	 */
	@GetMapping("/getBanner")
	@ApiOperation(value = "展示轮播图", tags = {"轮播图接口"}, notes = "展示轮播图")
	public RestResponseBean getBanner(@RequestParam Integer type){
		Banner banner = bannerService.queryByType(type);

		BannersVo bannersVo = new BannersVo();

		String imgUrl = banner.getImgUrl();
		Map mapTypes = JSON.parseObject(imgUrl);
		List<ImgUrl> list = new ArrayList<>();
		for (Object o : mapTypes.keySet()) {
			ImgUrl imgUrl1 = new ImgUrl();
			imgUrl1.setId((String) o);
			imgUrl1.setImgUrl((String) mapTypes.get(o));
			list.add(imgUrl1);
//			System.out.println(o);
//			System.out.println("value值：" + mapTypes.get(o));
		}
		BeanUtils.copyProperties(banner, bannersVo);
		bannersVo.setImgUrls(list);

		return new RestResponseBean(ResultEnum.OPERATION_SUCCESS.getValue(),ResultEnum.OPERATION_SUCCESS.getDesc(),bannersVo);
	}
...
//	@ApiOperation(value = "添加轮播图",tags = {"轮播图接口"}, notes = "添加轮播图")
	public Result<Banner> add(@RequestParam Integer type, @RequestParam String[] imgs,@RequestParam String[] ids ) {
		Result<Banner> result = new Result<Banner>();
		Banner banner = new Banner();
		banner.setImgUrl(JsonUtil.imgUrl(ids, imgs));
		banner.setType(type);
		try {
