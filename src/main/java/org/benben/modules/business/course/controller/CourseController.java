...
import org.benben.common.util.ExchangeRate;
...
public class CourseController {
	...
	public Result<Course> add(@RequestBody Course course) {
		...
		//人民币转换为港币
		BigDecimal chinaPrice = course.getChinaPrice();
		BigDecimal hkPrice = ExchangeRate.exchangeRate(chinaPrice);
		course.setHkPrice(hkPrice);
		try {
	...
	public Result<Course> queryById(@PathVariable String id) {
		Result<Course> result = new Result<Course>();
		Course course = courseService.getById(id);
		if(course==null) {
			result.error500("未找到对应实体");
  ...
  public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
      ...
      //导出文件名称
      mv.addObject(NormalExcelConstants.FILE_NAME, "关于课程的管理列表");
      mv.addObject(NormalExcelConstants.CLASS, Course.class);
      mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("关于课程的管理列表数据", "导出人:Jeecg", "导出信息"));
